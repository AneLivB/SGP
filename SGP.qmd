...
The error rate is calculated by comparing all scored alleles and loci from the standard procedure within one rack with the other treatments starting with the internal control. The internal control is included as a baseline for error rates to have a comparable metric to evaluate if the reused plates inflate the error rate. The error rate is calculated both as per-allele genotyping error rate and per locus genotyping error rate.
...
To further understand where the errors occur in the dataset, the error rate per mix across all racks were calculated.
...
# #to scale nanodrop to be on a (0,1) scale like the other varibles (aka Standardization)
...
plot(model.match)
pp_check(model.match, ndraws = 100)
...
#Build the plot
Density.match.plot <- ggplot(data = subset(model_info, parameter == c("Internal control", "Reused detection plate", "Reused PCR plate"))) +
  aes(x = x, y = parameter) +
  ...
  geom_vline(xintercept = 2.212687e+00, col = "#ca562c", linetype="longdash")+
  guides(fill = F, scale = "none") + #remove the legend
...
plot(model.missing)
pp_check(model.missing, ndraws = 100)
...
Density.missing.plot <- ggplot(data = subset(model_info, parameter == c("Standard procedure", "Internal control", "Reused detection plate", "Reused PCR plate"))) +
  ...
  geom_vline(xintercept = 6.3712923, col = "#ca562c", linetype="longdash")+
...
On R154 the two samples from 2007-2008 were excluded from analysis, due to lack of DNA for the last treatment. Likewise for two of the positive controls on R156. In total, `r Total_Mcases` alleles were analysed, out of which `r Total_Missing` were unscorable. We found a total of `r Total_Mcases` mismatches on allelic level between all treatments across all racks. A subset of these (`r Total_Ccases`) were potential contamination cases.
