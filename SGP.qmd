...
#| echo: FALSE
#here::here()
invisible(pacman::p_load(dplyr, tidyverse, here, stringi, png, knitr, rstatix, cowplot, kableExtra, brms, bayesplot))
```
...
R154_R4 <- Data_df %>% select(contains("Rack154")) %>% select(contains("_4")) %>%
  rename_with(~sub("Rack154_", "", .), everything()) %>%
  rename_with(~sub("_4", "", .), everything()) %>% ZcwCO1_loci() %>% write_csv(., file.path(working_dir, "R154_R4.csv"))
...
Exclude <- c("A04_AGP07487","A05_AGP07488","A01_AGP16315","A02_AGP16316")
Analysis.data <- subset(Analysis.data, ID != Exclude)
...
r = 10
#model bernoulli
model <- brms::brm(Match ~ Treatment + (1|ID),
             data = Analysis.data,
             family = bernoulli,
             cores = 8,
             iter = 1000*r,
             thin = 1*r,
             warmup = 1000, #update to 1000
             chains = 3,
...
model_interval <- mcmc_intervals_data(model)
...
#area
model_info <- mcmc_areas_data(model)
...
Densityplot <- ggplot(data = subset(model_info, parameter == c("Reused detection plate", "Reused PCR plate"))) + 
  aes(x = x, y = parameter) +
  geom_ridgeline(aes(scale = 0.4, height = scaled_density, fill = parameter, col = parameter), size = 0.9) +
  geom_segment(data = subset(model_interval, parameter != "Intercept"), aes(x = l, xend = h, yend = parameter), col = "black", size=2) +
  geom_segment(data = subset(model_interval, parameter != "Intercept"), aes(x = ll, xend = hh, yend = parameter), col = "black") +
  geom_point(data = subset(model_interval, parameter != "Intercept"), aes(x = m, y = parameter), color="black", fill = "grey60", shape=21, size = 3) + 
...
ggsave(plot = Densityplot, filename = file.path(figs_dir, "Densityplot.png"), width = 15, height = 10, unit = "cm", dpi = 500)
```
