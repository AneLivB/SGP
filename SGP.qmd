...
#| echo: FALSE
#here::here()
invisible(pacman::p_load(dplyr, tidyverse, here, stringi, png, knitr, rstatix, cowplot, kableExtra, brms, bayesplot, ggridges))
```
...
# Analysis.data <- Analysis.data %>% mutate(dummy=1) %>% spread(key=Treatment,value=dummy, fill=0) 
# colnames(Analysis.data)[10:12] <- c("R2", "R3", "R4")
# 
# #length(unique(Analysis.data$ID)) #285 individuals
...
# #length(unique(Analysis.data$ID)) #281 individuals
# 
# # #to scale nanodrop to be on a (0,1) scale like the other variables (aka Standardization)
# # range01 <- function(x){(x-min(x))/(max(x)-min(x))}
# # Analysis.data$StandardizedNanodrop <- range01(Analysis.data$Nanodrop)
# 
...
# #shapiro.test(scale(Nanodrop_SGP$Nanodrop)) #not normally distributed
# 
# #In R the intercept is called 1, you can remove it by adding -1 to your model
# set.seed(1995)
# model.match <- brms::brm(Match ~ R2 + R3 + R4 + (1|ID) + (1|Rack) + (1|Loci) + (1|Mix),
#              data = Analysis.data,
#              family = bernoulli,
#              cores = 10,
#              iter = 1000*r,
...
#              chains = 3,
#              control = list(adapt_delta = 0.99, max_treedepth = 10), #default max_treedepth = 10
#              set_prior("normal(0,1)", class = "b")) #standardized weakly informative prior
#                                                     #Prior for the random effect class = "sd", standard is normal(0, sigma_x)
# save(model.match, file = "Data/Working_data/Processed_data/model.match.Rdata")
# load(file="Data/Working_data/Processed_data/model.match.Rdata")
# pairs(model.match)
# summary(model.match)
...
#   parameter == "b_Intercept" ~ "Standard procedure",
#   parameter == "b_R2"  ~ "Internal control",
#   parameter == "b_R3" ~ "Reused detection plate",
#   parameter == "b_R4"  ~ "Reused PCR plate"))
# 
...
#   parameter == "b_R4"  ~ "Reused PCR plate")) 
# 
# model_info$parameter <- factor(model_info$parameter, levels = c("Standard procedure", "Internal control", "Reused detection plate", "Reused PCR plate"))
# # 
# # #Build the plot
# Density.match.plot <- ggplot(data = subset(model_info, parameter == c("Standard procedure", "Internal control", "Reused detection plate", "Reused PCR plate"))) +
#   aes(x = x, y = parameter) +
#   geom_ridgeline(data = subset(model_info, parameter == "Standard procedure"), aes(x = x, y = parameter, scale = 0.4, height = NA, fill = parameter, col = parameter), size = 0.9) +
#   geom_ridgeline(data = subset(model_info, parameter != "Standard procedure"), aes(scale = 0.4, height = scaled_density, fill = parameter, col = parameter), size = 0.9) +
#   geom_segment(data = model_interval[2:4,], aes(x = l, xend = h, yend = parameter), col = "black", size=2) +
#   geom_segment(data = model_interval[2:4,], aes(x = ll, xend = hh, yend = parameter), col = "black") +
#   geom_point(data = model_interval[2:4,], aes(x = m, y = parameter), color="black", fill = "grey60", shape=21, size = 3) +
#   geom_vline(xintercept = 2.2626087, col = "#ca562c", linetype="longdash")+
#   guides(fill = F, scale = "none") + #remove the legend
#   scale_fill_brewer(palette = "Oranges") +
#   scale_color_brewer(palette = "Oranges") +
#   ggtitle("A: Match model") +
#   ylab("Treatment") +
...
#
# #Create binary treatment variables
Analysis.data$R2 <- ifelse(Analysis.data$Treatment == "Internal control", 0.5, -0.5)
Analysis.data$R3 <- ifelse(Analysis.data$Treatment == "Re-used detection plate", 0.5, -0.5)
Analysis.data$R4 <- ifelse(Analysis.data$Treatment == "Re-used PCR plate", 0.5, -0.5)
# 
# #length(unique(Analysis.data$ID)) #285 individuals
# #We want to exclude the following individuals due to incorrect data
# #"A04_AGP07487" "A05_AGP07488" "A01_AGP16315" "A02_AGP16316" "AGF13004" "AGP16191" (Last two changed name to the original sample)
...
#In R the intercept is called 1, you can remove it by adding -1 to your model
set.seed(1995)
model.match <- brms::brm(Match ~ -1 + R2 + R3 + R4 + (1|ID) + (1|Rack) + (1|Loci) + (1|Mix),
             data = Analysis.data,
             family = bernoulli,
             cores = 3,
             iter = 1500*r,
             thin = r, #adjust thinning so you have at least 1000 samples per chain (check: (iter-warmup)/thin)
             warmup = 500*r, #standard is 20-50%
             chains = 3,
             control = list(adapt_delta = 0.99, max_treedepth = 10), #default max_treedepth = 10, stan does not recommend increasing
...
model_info$parameter <- factor(model_info$parameter, levels = c("Internal control", "Reused detection plate", "Reused PCR plate"))
#
# #Build the plot
Density.match.plot <- ggplot(data = model_info) +
  aes(x = x, y = parameter) +
  geom_ridgeline(aes(scale = 0.4, height = scaled_density, fill = parameter, col = parameter), size = 0.9) +
  geom_segment(data = model_interval, aes(x = l, xend = h, yend = parameter), col = "black", size=2) +
  geom_segment(data = model_interval, aes(x = ll, xend = hh, yend = parameter), col = "black") +
  geom_point(data = model_interval, aes(x = m, y = parameter), color="black", fill = "grey60", shape=21, size = 3) +
  geom_vline(xintercept = 0.3426995, col = "grey60", linetype="longdash")+
  guides(fill = F, scale = "none") + #remove the legend
  scale_fill_brewer(palette = "Oranges") +
  scale_color_brewer(palette = "Oranges") +
  ggtitle("B: Match model") +
  ylab("Treatment") +
  xlab("Posterior distribution") +
  xlim(-4,4) +
  theme_bw() +
...
# Analysis.data.missing <- Analysis.data.missing %>% mutate(dummy=1) %>% spread(key=Treatment,value=dummy, fill=0)
# colnames(Analysis.data.missing)[10:13] <- c("R2", "R3", "R4", "R1")
# 
# #A Bernoulli trial is a random experiment that has only two possible outcomes: success or failure.
# #Missing/not missing is similar, as we expect data not to be missing, is missing data is observed, that's a 'failure'
# 
# r = 100
# 
# #model bernoulli: Missing
# set.seed(2024)
# model.missing <- brms::brm(Missing ~ R1 + R2 + R3 + R4 + (1|ID) + (1|Rack) + (1|Loci) + (1|Mix),
...
# 
# save(model.missing, file = "Data/Working_data/Processed_data/model.missing.Rdata")
# load(file="Data/Working_data/Processed_data/model.missing.Rdata")
...
# # Plot the model
# #Extract info from model output
# model_interval <- mcmc_intervals_data(model.missing)
# model_interval <- subset(model_interval, !grepl("b_intercept", parameter))
# model_interval <- subset(model_interval, grepl("b_", parameter))
# model_interval <- model_interval %>% mutate(parameter = case_when(
#   parameter == "b_Intercept" ~ "Intercept",
#   parameter == "b_R1"  ~ "Standard procedure",
#   parameter == "b_R2"  ~ "Internal control",
#   parameter == "b_R3" ~ "Reused detection plate",
#   parameter == "b_R4"  ~ "Reused PCR plate"))
# 
# #area (for density clouds)
# model_info <- mcmc_areas_data(model.missing)
# model_info <- subset(model_info, !grepl("b_intercept", parameter))
# model_info <- subset(model_info, grepl("b_", parameter))
# model_info <- model_info %>% mutate(parameter = case_when(
#   parameter == "b_Intercept" ~ "Intercept",
...
# model_info$parameter <- factor(model_info$parameter, levels = c("Standard procedure", "Internal control", "Reused detection plate", "Reused PCR plate"))
# # #Build the plot
# Density.missing.plot <- ggplot(data = subset(model_info, parameter == c("Standard procedure", "Internal control", "Reused detection plate", "Reused PCR plate"))) +
#   aes(x = x, y = parameter) +
#   #geom_ridgeline(data = subset(model_info, parameter == "Standard procedure"), aes(x = x, y = parameter, scale = 0.4, height = scaled_density, fill = parameter, col = parameter), size = 0.9) +
#   geom_ridgeline(aes(scale = 0.4, height = scaled_density, fill = parameter, col = parameter), size = 0.9) +
#   geom_segment(data = model_interval[2:5,], aes(x = l, xend = h, yend = parameter), col = "black", size=2) +
#   geom_segment(data = model_interval[2:5,], aes(x = ll, xend = hh, yend = parameter), col = "black") +
#   geom_point(data = model_interval[2:5,], aes(x = m, y = parameter), color="black", fill = "grey60", shape=21, size = 3) +
#   geom_vline(xintercept = 6.5424092, col = "#ca562c", linetype="longdash")+
#   scale_y_discrete(limits = c("Standard procedure", "Internal control", "Reused detection plate", "Reused PCR plate")) +
#   guides(fill = F, scale = "none") + #remove the legend
#   scale_fill_brewer(palette = "Oranges") +
#   scale_color_brewer(palette = "Oranges") +
#   ggtitle("B: Missing genotypes model") +
#   #ylab("Treatment") +
#   xlab("Posterior distribution") +
...
#| echo: FALSE
#| warning: FALSE
#Densities.plot <- plot_grid(Density.match.plot, Density.missing.plot, align = "h", rel_widths = c(1, 1.2))

# ggsave(plot = Densities.plot, filename = file.path(figs_dir, "Densities.png"), width = 30, height = 15, unit = "cm", dpi = 500)
...
Missingdata$Missingrate <- (Missingdata$Total/(95*78))
...
**Match model** 
**Match \~ Internal control + Reused detection plate + Reused PCR plate + (1\|ID) + (1\|Rack) + (1\|Loci) + (1\|Mix)**
...
**Missing genotypes model** 
**Missing genotypes \~ Standard procedure + Internal control + Reused detection plate + Reused PCR plate + (1\|ID) + (1\|Rack) + (1\|Loci) + (1\|Mix)**

The first model yielded a significant effect of all treatments, where the Reused detection plate treatment had a positively diverged posterior distribution and the Reused PCR plate treatment yielded a negatively diverged posterior distributed in relation to the Internal control treatment (See @fig-Models A).

The second model yielded a significant effect of all treatments, where the Internal control treatment and the Reused detection plate treatment had a positively diverged posterior distribution and the Reused PCR plate treatment yielded a negatively diverged posterior distributed in relation to the Standard procedure (See @fig-Models B).

![Posterior distribution from models](Results/Figures/Densities.png){#fig-Models}
