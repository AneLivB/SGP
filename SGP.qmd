...
model.match <- brms::brm(Match ~ Treatment + (1|ID) + (1|Rack) + (1|Loci) + (1|Mix),
             ...
             cores = 3,
             iter = 100000,
             thin = 60, #adjust thinning so you have at least 1000 samples per chain (check: (iter-warmup)/thin)
             warmup = 40000, #standard is 20-50%
             chains = 3,
...
model_interval <- subset(model_interval, grepl("b_", parameter)) %>% select(-outer_width, -inner_width, -point_est)
...
m <- model_interval %>% subset(parameter == "Internal control") %>% select(m) #6.48881

#Add intercept values to estimates
Intercept <- model_interval %>% subset(parameter == "Internal control") 
Other <- model_interval  %>% subset(parameter != "Internal control") %>% mutate(.[2:6]+6.48881)
model_interval <- rbind(Intercept, Other)
...
model_info_intercept <- model_info %>% subset(parameter == "Internal control")
model_info_other <- model_info %>% subset(parameter != "Internal control") %>% mutate(x = x+6.4881)
model_info <- rbind(model_info_intercept, model_info_other)
...
Density.match.plot <- ggplot(data = model_info) +
  ...
  geom_point(data = model_interval, aes(x = m, y = parameter), color="black", fill = "grey60", shape=21, size = 2) +
...
#M value for intercept 
m <- model_interval %>% subset(parameter == "Internal control") %>% select(m) #5.15101

#Add intercept values to estimates
Intercept <- model_interval %>% subset(parameter == "Internal control") 
Other <- model_interval  %>% subset(parameter != "Internal control") %>% mutate(.[2:6]+5.15101)
model_interval <- rbind(Intercept, Other)
...
#add median value to other treatments 
model_info_intercept <- model_info %>% subset(parameter == "Internal control")
model_info_other <- model_info %>% subset(parameter != "Internal control") %>% mutate(x = x+5.15101)
model_info <- rbind(model_info_intercept, model_info_other)

# #Build the plot
Density.missing.plot <- ggplot(data = subset(model_info, parameter == c("Internal control", "Reused detection plate", "Reused PCR plate"))) +
  aes(x = x, y = parameter) +
  ...
  guides(fill = F, scale = "none") + #remove the legend
  scale_fill_brewer(palette = "Oranges") +
  ggtitle("A: Missing genotypes model") +
  #ylab("Treatment") +
  xlab("Estimate") +
  ylab(" ") +
  theme_bw() +
...
#| echo: FALSE
#| warning: FALSE
Densities.plot <- plot_grid(Density.missing.plot, Density.match.plot, align = "h", rel_widths = c(1.35, 1))

# ggsave(plot = Densities.plot, filename = file.path(figs_dir, "Densities.png"), width = 25, height = 10, unit = "cm", dpi = 500)
