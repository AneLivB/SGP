...
Analysisformat <- function(DF) {
  ...
  Base$Rack <- deparse(substitute(DF))
  Round <- gsub("R\\d{3}|_", "", deparse(substitute(DF)))
  if (Round == "R1"){
    Treatment <- "Standard procedure"
  } else if (Round == "R2") {
      "Internal control"
  } else if (Round == "R3") {
    Treatment <- "Re-used PCR"
