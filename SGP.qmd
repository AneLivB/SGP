...
Total_NA <- function(R1, R2, R3, R4) {
  ...
  Base$NAs <- NAs$NAs
   # Base$R1 <- as.numeric(R1$Row_Sums)
   # Base$R2 <- as.numeric(R2$Row_Sums)
   # Base$R3 <- as.numeric(R3$Row_Sums)
   # Base$R4 <- as.numeric(R4$Row_Sums)
   #Total <- Base[3:6] %>% mutate(Total = rowSums(across(everything())))
...
#  ggtitle("Missing data per individual across racks") +
...
```{r}
#| label: Analysisformat
#| echo: FALSE
#| 
#The function sets up the data for further analysis
Analysisformat <- function(DF1, DF2) {
   #Base is the underlying format
   Base <- DF1[1:2]
   colnames(Base) <- c("Well", "ID")
   Base$Row <- substring(Base$ID, 1, 1)
   Base$Column <- substring(Base$ID, 2, 3) 
   Base$Column <- as.integer(Base$Column)
   Base$ID <- gsub("^\\w{3}_", "", Base$ID)
   Base$Rack <- substring(deparse(substitute(DF2)), 1, 4) 
   ...
   #locate the mismatches
   mismatches <- which(DF1 != DF2, arr.ind = TRUE)
   #Match coded as 1, mismatch as 0
   Base$Match <- ifelse(Base$Well %in% mismatches[,1], 0, 1)
  ...
  mutate(Row_Sums = rowSums(across(everything())))
   ...
   Rack_1 <- deparse(substitute(DF1))
   Rack_2 <- deparse(substitute(DF2))
  #Make sure to add NA values for the rows that should be excluded from further analysis
  if (Rack_1 == "R154_R1") {
    #On plate R154 the samples in 4th and 5th row are not part of the analysis
    Base$Match[4:5] <- NA
    Base$Match.na[4:5] <- NA
  } else if (Rack_1 == "R156_R1"){
      #The repeats are not consistent across all plates, so we remove them
      Base$Match[1:2] <- NA
      Base$Match.na[1:2] <- NA
  } 
   return(Base)
}

Analysis.data <- rbind(Analysisformat(R154_R1, R154_R2),
              Analysisformat(R154_R1, R154_R3),
              Analysisformat(R154_R1, R154_R4),
              Analysisformat(R155_R1, R155_R2),
              Analysisformat(R155_R1, R155_R3),
              Analysisformat(R155_R1, R155_R4),
              Analysisformat(R156_R1, R156_R2),
              Analysisformat(R156_R1, R156_R3),
              Analysisformat(R156_R1, R156_R4))
```
